package DSFLib
  package Circuits "Library of electrical circuit components"
    package Units "Electrical variables"
      type Voltage = Real(unit = "V");
      type Current = Real(unit = "A");
      annotation(
        Documentation(info = "<html>
    <p>This package provides predefined types for electrical variables based on the international standard on units. </p>
    </html>"));
    end Units;
    ...
    package Interfaces
      ...
      partial model OnePort "Component with two pins p and n and current i from p to n"
        DSFLib.Circuits.Interfaces.Pin p "Positive pin" annotation(
        ...
        DSFLib.Circuits.Interfaces.Pin n "Negative pin" annotation(
        ...
        DSFLib.Circuits.Units.Voltage v "Voltage drop of the component";
        DSFLib.Circuits.Units.Current i "Current flowing from pin p to pin n";
      equation
        v = p.v - n.v;
        p.i = i;
        n.i + p.i = 0;
        annotation(
          Documentation(info = "<html>
      <p>Superclass of elements which have <strong>two</strong> electrical pins: the positive pin <em>p</em>, and the negative pin <em>n</em>. It is assumed that the current flowing into pin p is identical to the current flowing out of pin n. This current is provided explicitly as current i.</p>
      </html>"));
      end OnePort;
      annotation(
        Documentation(info = "<html>
    ...
    package Components "Electrical components"
      ...
      model Resistor "Linear electrical resistor"
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The linear resistor connects the voltage drop <em>v</em> with the current <em>i</em> by <em>i*R = v</em>. The Resistance <em>R</em> is allowed to be positive, zero, or negative.</p>
      </html>"),
      ...
      model Capacitor "Linear electrical capacitor"
        extends DSFLib.Circuits.Interfaces.OnePort;
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The linear capacitor connects the voltage drop <em>v</em> with the current <em>i</em> by <em>i = C * dv/dt</em>. The Capacitance <em>C</em> is allowed to be positive or zero.</p>
      </html>"),
      ...
      model Inductor "Linear electrical inductor"
        extends DSFLib.Circuits.Interfaces.OnePort;
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The linear inductor connects the voltage <em>v</em> with the current <em>i</em> by <em>v = L * di/dt</em>. The Inductance <em>L</em> is allowed to be positive or zero.</p>
      </html>"),
      ...
      model Diode "Simplified Diode"
        extends DSFLib.Circuits.Interfaces.OnePort;
        parameter Real Ron = 1e-5 "Forward state-on resistance (closed resistance)", Roff = 1e5 "Backward state-off resistance (opened resistance)", Vknee = 0.6 "Forward threshold voltage";
      equation
        i = if v > Vknee then (v - Vknee)/Ron + Vknee/Roff else v/Roff;
        annotation(
          Documentation(info = "<html>
      <p>
      The simplified diode is<br><br>
      <strong>open </strong>(off), if it is reversed biased (voltage drop less than Vknee).<br>
      <strong>closed</strong> (on), if it is forwarded biased (voltage drop greater than Vknee).<br>
      <br/>
      In order to prevent singularities during switching, the opened
      diode has a big resistance <em>Roff</em> and the closed diode has a low resistance <em>Ron</em>.
      </p>
      <p>
      The parameter <em>Vknee</em> which is the forward threshold voltage, allows to displace the knee point<br> along  the <em>V-A</em> characteristic.
      </p>
      </html>"),
      ...
      model Switch
        extends DSFLib.Circuits.Interfaces.OnePort;
        parameter Real Ron = 1e-5 "Forward state-on resistance (closed resistance)", Roff = 1e5 "Backward state-off resistance (opened resistance)";
        parameter Real dc = 0.5 "Duty cycle";
        parameter Real T = 1 "Switching period";
      equation
        v = if (time/T) - integer(time/T) < dc then i*Ron else i*Roff;
        annotation(
          Documentation(info = "<html>
      <p>The Switch is a conmutated element. Given a period T and a duty cycle dc, the switch remains closed for T*dc and opened for T*(1-dc), repeating this behaveour in every cycle. In order to prevent singularities during switching, the opened state has a big resistance <em>Roff</em> and the closed state has a low resistance <em>Ron</em>.
      </html>"),
      ...
      model ConstCurr "Source for constant current"
        extends DSFLib.Circuits.Interfaces.OnePort;
        parameter Real I(unit = "A") = 1 "Value of constant current";
      equation
        i = -I;
        annotation(
          Documentation(info = "<html>
      <p>The ConstantCurr source is a simple source for an ideal constant current which is provided by a parameter.
      </html>"),
      ...
      model ConstVolt "Source for constant voltage"
        extends DSFLib.Circuits.Interfaces.OnePort;
        parameter Real V(unit = "V") = 1 "Value of constant voltage";
      equation
        v = V;
        annotation(
          Documentation(info = "<html>
      <p>The ConstantVolt source is a simple source for an ideal constant voltage which is provided by a parameter.</p>
      </html>"),
      ...
      model NLInductor "Non linear inductor"
        extends DSFLib.Circuits.Interfaces.OnePort;
        parameter Real[:] currTable = {-2, -1, 0, 1, 2} "Current values";
        parameter Real[:] fluxTable = {-2, -1, 0, 1, 2} "Flux values";
        Real phi(unit = "V.s");
      equation
        der(phi) = v;
        ...
        annotation(
          Documentation(info = "<html>
      <p>This model approximates the behavior of a non linear inductor, where the current-flux relation is obtained from the values in currTable and fluxTable. Values that are not explicitly contained in the tables are calculated using linear interpolation. Note that hysteresis is not taken into account.
      </p>
      </html>"),
    ...
    package Examples "Examples that demostrate the usage of the circuit components"
      model RLC "RLC serie circuit"
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>This example is a RLC serie circuit without sources. The capacitor has an initial charge.
      </p>
      </html>"),
          experiment(StartTime = 0, StopTime = 0.02, Tolerance = 1e-06, Interval = 4.00802e-05));
      end RLC;

      package SolarPanels "Solar panels"
        package Components
          model SolarCell "Solar cell"
          ...
          protected
            annotation(
              Documentation(info = "<html>
          <p>The solar cell is modeled by a current source, a diode and two resistors.
          </p>
          </html>"),
              Diagram,
          ...
          equation
            ...
            annotation(
              Documentation(info = "<html>
          <p> The solar panel is modeled as an NxM array of solar cells. It has M columns connected in parallel, and each column consists of N solar cells connected in series.
          </p>
          </html>"),
          ...
          annotation(
            Documentation(info = "<html>
          <p>This package contains the model of a solar cell and a solar panel.
          </p>
          </html>"));
        end Components;

        model TwoCells "Two solar cells with load"
          DSFLib.Circuits.Components.Ground ground annotation(
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p> This example shows a circuit with two solar cells in serie connected to a RC load.
        </p>
        </html>"),
            experiment(StartTime = 0, StopTime = 0.02, Tolerance = 1e-06, Interval = 4.00802e-05));
        end TwoCells;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p> This example shows a circuit with a solar panel <em>(10x5)</em> connected to a RC load.
        </p>
        </html>"),
        ...
        annotation(
          Documentation(info = "<html>
      <p>This example contains the model of a solar panel and its use with a load.
      </p>
      </html>"));
      end SolarPanels;
  ...
  package Mechanical "Library of 1-dim. and 2-dim. mechanical components (rotational, translational and planar)"
    package Translational "Library to model 1-dimensional, translational mechanical systems"
      package Units "Mechanical Translational (1-dim.) variables"
      ...
      package Components "Components for 1D translational mechanical systems"
        import DSFLib.Mechanical.Translational.Units.*;
        ...
        model ConstForce "Constant force"
          DSFLib.Mechanical.Translational.Interfaces.Flange flange annotation(
      ...
      package Examples "Examples that demostrate the usage of the mechanical translational components"
        import DSFLib.Mechanical.Translational.Components.*;

        model SpringMass "Sprin mass system"
        ...
        model SpringMassForce "Forced spring mass system"
          DSFLib.Mechanical.Translational.Components.Fixed fixed annotation(
            Placement(visible = true, transformation(origin = {-22, 0}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
        ...
        model SpringDamperMass "Damped spring mass"
          DSFLib.Mechanical.Translational.Components.Fixed fixed annotation(
    ...
    package Rotational "Library to model 1-dimensional, rotational mechanical systems"
      package Units "Mechanical Rotational (1-dim.) variables"
      ...
      package Interfaces
        ...
        partial model Compliant "Compliant connection of two rotational 1D flanges"
      ...
      package Components "Components for 1D rotational mechanical systems"
        ...
        model ConstTorque "Constant torque"
      ...
      package Examples "Examples that demostrate the usage of the mechanical rotational components"
        import DSFLib.Mechanical.Rotational.Components.*;

        model SpringDamperInertia "Damped spring inertia"
    ...
    package RotoTranslational "Library to model roto-translational mechanical systems"
      package Components "Components for roto-translational mechanical systems"
        model RackPinion "Rack-Pinion"
      ...
      package Examples "Examples that demostrate the usage of the roto-translational components"
        model RotTrans "Roto-translational system example"
    ...
    package Planar "Library to model planar mechanical systems"
      package Units "Mechanical planar (2-dim.) variables"
      ...
      package Interfaces
        ...
        partial model Compliant "Compliant connection of two planar (2D) flanges"
      ...
      package Components "Components for planar mechanical systems"
        model Fixed "Planar fixed"
        ...
        model pointMass "Planar point Mass"
          DSFLib.Mechanical.Planar.Interfaces.Frame frame annotation(
      ...
      package Examples "Examples that demostrate the usage of the planar components"
        ...
        model PhysicalPendulum "Physical pendulum"
        ...
        model DoublePendulum "Doble Péndulo"
          DSFLib.Mechanical.Planar.Components.Revolute revolute annotation(
        ...
        model BridgeCrane "Bridge crane"
          DSFLib.Mechanical.Planar.Components.Revolute revolute annotation(
        ...
        model SpringPendulum "Spring Pendulum"
          DSFLib.Mechanical.Planar.Components.Revolute revolute annotation(
        ...
        model SliderCrank "Slider crank"
          DSFLib.Mechanical.Planar.Components.Revolute revolute annotation(
  ...
  package Hydraulics "Library of hydraulic components"
  ...
  This model represents a simplified hydraulic tank used to store incompressible fluid in hydraulic circuits. The tank is characterized by a fixed cross-sectional area and assumes uniform fluid pressure and density throughout. The pressure is calculated based on the hydrostatic relation, considering the fluid height, gravity, and density. In order to avoid a negative volume, if the volume is zero, a high value valve is added.
      ...
      model ConstPress "Constant pressure"
    ...
    package Examples "Examples that demostrate the usage of the hydraulic components"
  ...
  This model represents an ideal thermal capacitor, a fundamental component in thermal systems used to store and release thermal energy. The thermal capacitor is characterized by its thermal capacity C (in J/K), which defines the amount of heat required to change its temperature by one kelvin.</p>
    ...
    package Examples "Examples that demostrate the usage of the thermal components"
  ...
  package MultiDomain "Multidomain"
    package ElectroMechanical "Electromechanical convertion"
      package Components "Components to model electromechanical convertion"
        import DSFLib.Circuits.Interfaces.*;
        import DSFLib.Circuits.Units.*;
        import DSFLib.Mechanical.Rotational.Interfaces.*;
        import DSFLib.Mechanical.Rotational.Units.*;

        model EMF "Permanent magnet electromechanical convertion"
          DSFLib.Circuits.Interfaces.Pin p annotation(
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
            <p>
            This component models the electromechanical conversion of a permanent magnet DC motor. The voltage induced (e) is proportional to the rotational speed (w) via constant K. Similarly, the generated torque (tau) is proportional to the input current (i) and the same constant. This component captures the bidirectional energy conversion between electrical and mechanical domains.
            </p>
            </html>"),
        ...
        model SepExcDCM "Separately excitation electromechanical convertion"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
            <p>
            This component models the electromechanical conversion of a separately exitation DC motor. The voltage induced (e) is proportional to the rotational speed (w) via constant K and the magnetic flux. Similarly, the generated torque (tau) is proportional to the input current (i), the flux and the same constant. This component captures the bidirectional energy conversion between electrical and mechanical domains.
            </p>
            </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a permanent magnet DC motor. The electrical side includes two pins that can be connected to a DC source, the armature resistance, and the armature inductance. The EMF represents the electromechanical conversion. On the mechanical side, the rotational inertia represents the rotor and the model includes a flange to connect a mechanical load.</p>
        </html>"),
        ...
        model DCMotorSepExc "Separately excitation DC motor"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a separately excited DC motor. The model is divided into two electrical circuits: the field circuit and the armature circuit. The field circuit consists of two pins that can be connected to a DC (excitation) source , the excitation resistance (Re), and a non linear inductor representing the excitation winding. The armature circuit includes anothers two pins that can be connected to a DC (armature) source, the armature resistance, and the armature inductance. The electromechanical conversion is modulated by the magnetic flux. On the mechanical side, the rotational inertia represents the rotor and the model includes a flange to connect a mechanical load.</p>
        </html>"),
      ...
      package Examples "Examples that demostrate the usage of the electromechanical components"
        model DCMotor "Permanent magnet DC motor with load"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a permanent magnet DC motor. The electrical side consists of voltage source, the armature resistance (R), and the armature inductance (L). The EMF represents the electromechanical conversion. On the mechanical side, the rotational inertia (I) represents a load.</p>
        </html>"),
            experiment(StartTime = 0, StopTime = 0.1, Tolerance = 1e-06, Interval = 0.0002));
        end DCMotor;

        model DCMotorSepExc "Separately excitation DC motor with load"
          DSFLib.Mechanical.Rotational.Components.Fixed fixed annotation(
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a separately excited DC motor. The model is divided into two electrical circuits: the field circuit and the armature circuit. The field circuit consists of two pins that can be connected to a DC (excitation) source , the excitation resistance (Re), and a non linear inductor representing the excitation winding. The armature circuit includes anothers two pins that can be connected to a DC (armature) source, the armature resistance, and the armature inductance. The electromechanical conversion is modulated by the magnetic flux. On the mechanical side, the rotational inertia represents the load, while the damper models the viscous friction losses.</p>
        </html>"),
            experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02),
            Diagram);
        end DCMotorSepExc;

        package SoftStartDCMotor "Soft start DC motor"

          model RampVolt "Ramp voltage source"
          extends DSFLib.Circuits.Interfaces.OnePort;
          parameter Real V(unit = "V") = 1 "Final voltage";
...
if time < t0 then
          ...
          <p>The  RamptVolt source is an ideal voltage source. The voltage is a ramp with a final value <strong>V</strong>. The growth starts at time <strong>t0</strong> and ends at time <strong>tf</strong>. The initial value is zero.
          </p>
          </html>"),
              Diagram(graphics),
  Icon(graphics = {Ellipse(lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, lineThickness = 0.3, extent = {{-50, 50}, {50, -50}}), Line(points = {{-80, 20}, {-60, 20}}, color = {0, 0, 255}), Line(points = {{60, 20}, {80, 20}}, color = {0, 0, 255}), Text(textColor = {0, 0, 255}, extent = {{-150, 70}, {150, 110}}, textString = "%name"), Line(points = {{-70, 30}, {-70, 10}}, color = {0, 0, 255}), Line( origin = {14, 0},points = {{-47, -23}, {-27, -23}, {0, 20}, {20, 20}}, color = {26, 95, 180})}));
end RampVolt;
           
model DCMotorSepExc2 "Soft start of separately excitation DC motor"
            MultiDomain.ElectroMechanical.Components.DCMotorSepExc motor(Rexc = 25.2, b = 1.1, phi0 = 500) annotation(
            ...
            Circuits.Components.ConstVolt Vexc(V = 184) annotation(
              Placement(transformation(origin = {-72, 0}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  RampVolt rampVolt(V = 460, tf = 5, t0 = 2)  annotation(
              Placement(transformation(origin = {-6, 58}, extent = {{-14, -14}, {14, 14}}, rotation = -90)));
          equation
            connect(motor.p_exc, Vexc.p) annotation(
              Line(points = {{-28, 10}, {-72, 10}}));
            connect(Vexc.n, motor.n_exc) annotation(
              Line(points = {{-72, -10}, {-72, -22}, {-28, -22}}));
  connect(rampVolt.n, motor.n) annotation(
              Line(points = {{-6, 44}, {-6, 30}}));
  connect(rampVolt.p, motor.p) annotation(
              Line(points = {{-6, 72}, {-6, 86}, {32, 86}, {32, 30}}));
            annotation(
              Documentation(info = "<html>
          <p>This model uses the DCMotorSepExc component along with a ramp voltage source to implement a soft start mechanism, which helps to avoid current peaks. The excitation circuit is powered by a constant voltage source.</p>
          </html>"),
              experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
          end DCMotorSepExc2;
        end SoftStartDCMotor;
      end Examples;
    end ElectroMechanical;

    package HydroMechanical "Hydromechanical convertion"
      package Components "Components to model hydromechanical convertion"
        import DSFLib.Hydraulics.Units.*;
        import DSFLib.Hydraulics.Interfaces.*;
        import DSFLib.Mechanical.Translational.Units.*;
        import DSFLib.Mechanical.Translational.Interfaces.*;

        model PistonCylinder "Piston-Cylinder"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
                <p>
                This component models the hydromechanical conversion of a piston-cylinder system. The force applied to the piston generates a proportional pressure in the fluid inside the cylinder. Similarly, there is a proportional relationship between the piston’s velocity and the resulting fluid flow rate. In both cases, the proportionality constant is the piston area.</p>
                </html>"),
      ...
      package Examples "Examples that demostrate the usage of the hydromechanical components"
        model HydraulicJack "Hydraulic jack"
          ...
          DSFLib.Mechanical.Translational.Components.ConstForce constForce1(F = -9.8e3) annotation(
            Placement(transformation(origin = {56, 18}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
        equation
          ...
          annotation(
            Documentation(info = "<html>
                <p>This model represents a hydraulic jack system used for force multiplication. A constant mechanical force is applied to one piston, generating pressure in the hydraulic fluid. This pressure is transmitted through the fluid to the other side of the system. The valve component models the pressure losses within the circuit. A mass is connected to the second piston to simulate the load being lifted.</p>
                </html>"),
            experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
        end HydraulicJack;
      end Examples;
    ...
    package HydroThermal "Hydrothermal"
      package Components "Components to model hydrothermal systems"
        import DSFLib.Thermal.Units.*;
        import DSFLib.Thermal.Interfaces.*;
        import DSFLib.Hydraulics.Units.*;
        import DSFLib.Hydraulics.Interfaces.*;
        ...
        model Convection
          ...
          HeatFlow q_heat "Heat flow from a to b";
          parameter Real rho(unit = "Kg/m3") = 1000 "Fluid density";
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
    <p>This component models convective heat transfer through a fluid stream. It assumes a fixed fluid density (rho) and specific heat capacity (c). The direction of heat flow depends on the flow direction (q). If the flow is positive, the inlet temperature is taken from heatPort_b, otherwise from heatPort_a. The heat flow (q_heat) is calculated based on the mass flow rate, fluid properties, and the temperature at the inlet.</p>
    </html>"),
      ...
      package Examples "Examples that demostrate the usage of the hydrothermal components"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a solar collector system. The collector consists of a static fluid volume that receives heat through solar radiation and loses heat to the environment. An ideal pump circulates fluid through the system, which gains energy by exchanging heat with the collector. The convective blocks simulate heat transfer between the circulating fluid and two rooms maintained at constant temperature.</p>
        </html>"));
        end SolarCollector;
      end Examples;
    end HydroThermal;

    package ElectroThermal "Electrothermal convertion"
      import DSFLib.Circuits.Components;
      import DSFLib.Thermal.Interfaces;

      package Components "Components to model electrothermal convertion"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This component models the heat generated by the Joule effect in an electrical resistor. It is considered that all electrical power is dissipated as heat.</p>
        </html>"),
      ...
      package Examples "Examples that demostrate the usage of the electrothermal components"
        model WaterHeater "Water heater"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a water heater system. The water is heated by a resistor. An ideal pump circulates the heated water through the system. The convective blocks simulate heat transfer between the circulating fluid and two rooms maintained at constant temperature.</p>
        </html>"),
            Diagram,
    ...
    package Examples "Examples that demonstrate the usage of the control systems components"
      ...
      package QuarterVehicle
        model QuarterVehiclePasiveSuspention "Quarter vehicle pasive suspention"
          ...
          Mechanical.Translational.Components.Spring k(k = 19960, s_rel0 = 0.7) annotation(
            Placement(visible = true, transformation(origin = {-16, 28}, extent = {{-10, 10}, {10, -10}}, rotation = 90)));
          Mechanical.Translational.Components.Damper c(b = 1300) annotation(
            Placement(visible = true, transformation(origin = {16, 28}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
          Mechanical.Translational.Components.Spring kt(k = 155900, s_rel0 = 0.4) annotation(
            Placement(visible = true, transformation(origin = {0, -38}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
          Mechanical.Translational.Components.ConstForce wc(F = -9.8*300) annotation(
            Placement(visible = true, transformation(origin = {44, 28}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
          Mechanical.Translational.Components.ConstForce wr(F = -9.8*30) annotation(
            Placement(visible = true, transformation(origin = {28, -38}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
          ModulatedPosition modulatedPosition annotation(
        ...
        block SoftPulseSource "Soft pulse signal source"
          ...
          ControlSystems.Blocks.Components.RampSource up(U = U, t0 = t1, tf = t2) annotation(
            Placement(visible = true, transformation(origin = {14, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
          ControlSystems.Blocks.Components.RampSource down(U = U, t0 = t3, tf = t4) annotation(
            Placement(visible = true, transformation(origin = {14, -16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
          ControlSystems.Blocks.Components.Add add(k2 = -1) annotation(
      ...
      package TwoInertiasSystem
        model TwoInertiasSystem "Two inertias system"
          ...
          TauSource tauSource annotation(
  ...
  package BondGraph "Library of Bond Graph"
    package Interfaces
      connector BondCon "Bond connector"
        Real e;
        flow Real f;
        output Integer d;
        annotation(
          Documentation(info = "<html>
      <p>The BondCon is the connector for bond graph components. It includes an effort variable (e) and a flow variable (f). An additional variable, d, defines the orientation of the bond, indicating whether the contribution of e and f should be added or subtracted. In this case, d is defined as an output, and therefore it must be calculated inside the component.</p>
      </html>"),
      ...
      partial model PassiveOnePort "Passive one port"
        BondGraph.Interfaces.BondCon bondCon annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The PassiveOnePort is a partial model for passive components, meaning they consume power. The variable d is assigned a value of -1 because the bond is oriented outward. </p> </html>"),
          Icon(graphics = {Line(origin = {-38.15, -20.21}, points = {{-79.8536, 20.2071}, {80.1464, 20.2071}, {40.1464, 60.2071}}, thickness = 10), Text(origin = {99, -82}, textColor = {0, 85, 255}, extent = {{-81, 20}, {81, -20}}, textString = "%name", fontSize = 12)}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
      end PassiveOnePort;

      partial model ActiveOnePort "Active one port"
        BondGraph.Interfaces.BondCon bondCon annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The ActiveOnePort is a partial model for active components, meaning they supply power. The variable d is assigned a value of 1 because the bond is oriented inward. </p> </html>"),
          Icon(graphics = {Line(origin = {59.09, -21.61}, points = {{-79.8536, 20.2071}, {80.1464, 20.2071}, {40.1464, 60.2071}}, thickness = 10), Text(origin = {-84, -80}, textColor = {0, 85, 255}, extent = {{-80, 22}, {80, -22}}, textString = "%name", fontSize = 12)}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
      end ActiveOnePort;

      partial model TwoPort "Two port"
        BondGraph.Interfaces.BondCon bondCon1 annotation(
        ...
        BondGraph.Interfaces.BondCon bondCon2 annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The TwoPort is a partial model for two-port components, such as transformers and gyrators. It includes two BondCon connectors: one with d = 1 and the other with d = -1, because the component consumes power on one side and supplies power on the other.</p> </html>"),
          Icon(graphics = {Line(origin = {-100.19, -18.42}, points = {{-101.854, 18.2071}, {18.1464, 18.2071}, {-23.8536, 60.2071}}, thickness = 10), Line(origin = {181.85, -17.37}, points = {{-101.854, 18.2071}, {18.1464, 18.2071}, {-23.8536, 60.2071}}, thickness = 10)}));
      end TwoPort;

      model ModPassiveOnePort "Modulated passive one port"
        extends BondGraph.Interfaces.PassiveOnePort;
        ControlSystems.Blocks.Interfaces.RealInput u annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The ModPassiveOnePort is a partial model for modulated passive components, meaning they consume power. This model includes a real input that can be used to modify the value of a parameter dynamically. The variable d is assigned a value of -1 because the bond is oriented outward. </p> </html>"));
      end ModPassiveOnePort;

      model ModActiveOnePort "Modulated active one port"
        extends BondGraph.Interfaces.ActiveOnePort;
        ControlSystems.Blocks.Interfaces.RealInput u annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The ActiveOnePort is a partial model for modulated active components, meaning they supply power. This model includes a real input that can be used to modify the value of a parameter dynamically. The variable d is assigned a value of 1 because the bond is oriented inward. </p> </html>"));
      end ModActiveOnePort;

      model ModTwoPort "Modulated two port"
        extends BondGraph.Interfaces.TwoPort;
        DSFLib.ControlSystems.Blocks.Interfaces.RealInput u annotation(
      ...
      equation
        ...
        annotation(
          Documentation(info = "<html>
      <p>The ModTwoPort is a partial model for modulated two-port components, such as transformers and gyrators. It includes two BondCon connectors: one with d = 1 and the other with d = -1, because the component consumes power on one side and supplies power on the other. Furthermore it incorporates a real input that can be used to modify the value of a parameter dynamically. </p> </html>"));
      end ModTwoPort;

      connector BondConJ "Bond junction connector"
        Real e;
        flow Real f;
        input Integer d;
        annotation(
          Documentation(info = "<html>
      <p>The BondCon is the connector for bond graph structural elements. It includes an effort variable (e) and a flow variable (f). An additional variable, d, defines the orientation of the bond, indicating whether the contribution of e and f should be added or subtracted. In this case, d is defined as an input, and therefore it must be calculated outside the component. </p>
      </html>"),
          Icon(graphics = {Ellipse(origin = {1, -3}, fillColor = {216, 216, 216}, pattern = LinePattern.None, fillPattern = FillPattern.Solid, extent = {{-99, 99}, {99, -99}}), Ellipse(origin = {4, -4}, fillColor = {255, 255, 255}, pattern = LinePattern.None, fillPattern = FillPattern.Solid, extent = {{-54, 52}, {54, -52}})}));
      end BondConJ;

      partial model Junction "Junction component"
        parameter Integer N = 3 "Number of ports";
        BondConJ bondConJ1(f = -f[1], e = e[1], d = d[1]) annotation(
          Placement(transformation(origin = {-100, 2}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {-100, 2}, extent = {{-10, -10}, {10, 10}})));
        BondConJ bondConJ2(f = -f[2], e = e[2], d = d[2]) annotation(
          Placement(transformation(origin = {100, -2}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}})));
        BondConJ bondConJ3(f = -f[3], e = e[3], d = d[3]) if N > 2 annotation(
          Placement(transformation(origin = {110, 8}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {0, 98}, extent = {{-10, -10}, {10, 10}})));
        BondConJ bondConJ4(f = -f[4], e = e[4], d = d[4]) if N > 3 annotation(
          Placement(transformation(origin = {120, 18}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {0, -98}, extent = {{-10, -10}, {10, 10}})));
        Real f[N], e[N];
        input Integer d[N];
        annotation(
          Documentation(info = "<html>
      <p>The Junction is a partial model used to instantiate 0-junctions or 1-junctions. It can have 2, 3, or 4 connected components depending on the value of N.</p>
      </html>"));
      end Junction;
      annotation(
        Documentation(info = "<html>
    <p>This package contains connectors and interfaces (partial model) for bond graph components. </p>
    </html>"));
    end Interfaces;

    package Components "Bond graph components"
      package Basic "Basic components"
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>Bond model, it includes two BondCon connectors: one with d = 1 and the other with d = -1  </p>
        </html>"),
            Icon(graphics = {Line(origin = {-0.46, -20.81}, points = {{-79.8536, 20.2071}, {80.1464, 20.2071}, {40.1464, 60.2071}}, thickness = 10)}));
        end Bond;

        model J0 "0-Junction"
          extends BondGraph.Interfaces.Junction;
        equation
          for i in 2:N loop
            e[i] = e[1];
          end for;
          ...
          annotation(
            Documentation(info = "<html>
        <p> In 0-junction all bond conectors share the same e variable.</p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p> In 1-junction all bond conectors share the same f variable.</p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The resistor represents energy dissipation. It establishes a static (linear) relationship between the flow and the effort. </p>
        </html>"),
            Icon(graphics = {Text(origin = {92, 6}, extent = {{-52, 92}, {52, -92}}, textString = "R")}));
        end R;

        model C "Linear capacitor"
          extends BondGraph.Interfaces.PassiveOnePort;
          parameter Real C = 1;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The capacitor represents energy storage. It establishes a static (linear) relationship between the flow integral (generalized displacement) and the effort. </p>
        </html>"),
            Icon(graphics = {Text(origin = {98, 6}, extent = {{-52, 92}, {52, -92}}, textString = "C")}));
        end C;

        model I "Linear Inertia"
          extends BondGraph.Interfaces.PassiveOnePort;
          parameter Real I = 1;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The inertia represents energy storage. It establishes a static (linear) relationship between the flow and the effort inegral (generalized momentum). </p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The effort source supplies energy. It sets a constant effort at its connector. </p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The flow source supplies energy. It sets a constant flow at its connector. </p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The Transformer represents power conversion. It establishes a static relationship between the efforts on both sides (and also between the flows). Note that power is conserved, as shown by the equality e1*f1=e2*f2.</p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The Gyrator represents power conversion. It establishes a static relationship between the effort of one side and the flow of the other. Note that power is conserved, as shown by the equality e1*f1=e2*f2.</p>
        </html>"),
            Icon(graphics = {Text(origin = {-3, 5}, extent = {{-69, 83}, {69, -83}}, textString = "GY")}));
        end GY;
      end Basic;

      package Modulated "Modulated components"
        model MR "Modulated linear resistor"
          extends BondGraph.Interfaces.ModPassiveOnePort;
        equation
          e = u*f;
          annotation(
            Documentation(info = "<html>
        <p>This model represents a resistor whose parameter is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {92, 6}, extent = {{-52, 92}, {52, -92}}, textString = "R")}));
        end MR;

        model MC "Modulated linear capacitor"
          extends BondGraph.Interfaces.ModPassiveOnePort;
          parameter Real q_0 = 0;
          Real q(start = q_0);
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a capacitor whose parameter is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {98, 6}, extent = {{-52, 92}, {52, -92}}, textString = "C")}));
        end MC;

        model MI "Modulated linear inertia"
          extends BondGraph.Interfaces.ModPassiveOnePort;
          parameter Real p_0 = 0;
          Real p(start = p_0);
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents an inertia whose parameter is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {98, 6}, extent = {{-52, 92}, {52, -92}}, textString = "I")}));
        end MI;

        model MSe "Modulated effort source"
          extends BondGraph.Interfaces.ModActiveOnePort;
        equation
          e = u;
          annotation(
            Documentation(info = "<html>
        <p>This model represents an effort source whose value is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {-92, 9}, extent = {{-64, 107}, {64, -107}}, textString = "Se")}));
        end MSe;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents an flow source whose value is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {-92, 9}, extent = {{-64, 107}, {64, -107}}, textString = "Sf")}));
        end MSf;

        model MTF "Modulated transformer"
          extends BondGraph.Interfaces.ModTwoPort;
        equation
          e1 = u*e2;
          f2 = u*f1;
          annotation(
            Documentation(info = "<html>
        <p>This model represents a transformer whose parameter is set by an external input signal.</p>
        </html>"),
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This model represents a gyrator whose parameter is set by an external input signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {-3, 5}, extent = {{-69, 83}, {69, -83}}, textString = "GY")}));
        end MGY;
      end Modulated;
      ...
      package Sensors "Sensors"
        model EffortSensor "Effort sensor"
          extends BondGraph.Interfaces.PassiveOnePort;
          DSFLib.ControlSystems.Blocks.Interfaces.RealOutput y annotation(
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The effort sensor acts as a zero-flow source. By connecting it to a 0-junction, the effort can be measured and output as a real-valued signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {92, 6}, extent = {{-52, 92}, {52, -92}}, textString = "De")}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
        end EffortSensor;

        model FlowSensor "Flow sensor"
          extends BondGraph.Interfaces.PassiveOnePort;
          DSFLib.ControlSystems.Blocks.Interfaces.RealOutput y annotation(
            Placement(visible = true, transformation(origin = {164, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {164, -2.22045e-15}, extent = {{-14, -14}, {14, 14}}, rotation = 0)));
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>The flow sensor acts as a zero-effort source. By connecting it to a 1-junction, the flow can be measured and output as a real-valued signal.</p>
        </html>"),
            Icon(graphics = {Text(origin = {92, 6}, extent = {{-52, 92}, {52, -92}}, textString = "Df")}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
        end FlowSensor;
      end Sensors;

      package MultiDomain "Multidomain components"
        model BGCircuit "Bond Graph/Circuits"
          extends BondGraph.Interfaces.PassiveOnePort;
          Circuits.Interfaces.Pin p annotation(
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This component converts the effort and flow variables at the Bond Graph connector into the potential difference and current at the electric pin, respectively.</p>
        </html>"));
        end BGCircuit;
      end MultiDomain;
      annotation(
        Documentation(info = "<html>
    <p>This package contains bond graph components such passive components, active components, gyrator, transformer and junctions.</p>
    </html>"));
    end Components;

    package Examples "Examples that demonstrate the usage of the Bond Graph components"
      model RC "RC circuit"
        Components.Basic.J1 j1 annotation(
          Placement(transformation(origin = {-2, 4}, extent = {{-10, -10}, {10, 10}})));
        Components.Basic.Se se annotation(
          Placement(transformation(origin = {-32, 4}, extent = {{-10, -10}, {10, 10}})));
        Components.Basic.R r annotation(
          Placement(transformation(origin = {24, 4}, extent = {{-10, -10}, {10, 10}})));
        Components.Basic.C c annotation(
          Placement(transformation(origin = {-2, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
      equation
        connect(se.bondCon, j1.bondConJ1) annotation(
          Line(points = {{-16, 4}, {-12, 4}}));
        connect(j1.bondConJ2, r.bondCon) annotation(
          Line(points = {{8, 4}, {12, 4}}));
        connect(j1.bondConJ3, c.bondCon) annotation(
          Line(points = {{-2, 14}, {-2, 18}}));
        annotation(
          Documentation(info = "<html>
      <p>This Bond Graph represents an RC circuit with a voltage source. The capacitor (C), the resistor (R) and the effort source (Se) are connected to a 1-junction, which enforces the same flow through them, representing the series connection.</p>
      </html>"),
          experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
      end RC;

      model RLCCircuit "RLC circuit"
        BondGraph.Components.Basic.J0 v1(N = 2) annotation(
        ...
        BondGraph.Components.Basic.Bond bond2 annotation(
          Placement(transformation(origin = {0, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
        BondGraph.Components.Basic.C C(q_0 = 1) annotation(
          Placement(transformation(origin = {-64, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
        BondGraph.Components.Basic.R R annotation(
          Placement(transformation(origin = {72, 0}, extent = {{-10, -10}, {10, 10}})));
        BondGraph.Components.Basic.R R2 annotation(
          Placement(transformation(origin = {42, 36}, extent = {{-10, -10}, {10, 10}})));
        BondGraph.Components.Basic.I L annotation(
          Placement(transformation(origin = {46, -26}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
        Components.Basic.J1 j1 annotation(
          Placement(transformation(extent = {{-10, -10}, {10, 10}})));
        Components.Basic.J0 v1_v2(N = 2) annotation(
          Placement(transformation(origin = {12, 36}, extent = {{-12, -12}, {12, 12}})));
        Components.Basic.J0 v2 annotation(
          Placement(transformation(origin = {46, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
      equation
        connect(C.bondCon, v1.bondConJ1) annotation(
          Line(points = {{-52, 0}, {-50, 0}}));
        connect(v1.bondConJ2, bond.bondCon1) annotation(
          Line(points = {{-30, 0}, {-28, 0}}));
        connect(bond.bondCon2, j1.bondConJ1) annotation(
          Line(points = {{-10, 0}, {-10, 0}}));
        connect(j1.bondConJ2, bond1.bondCon1) annotation(
          Line(points = {{10, 0}, {16, 0}}));
        connect(j1.bondConJ3, bond2.bondCon1) annotation(
          Line(points = {{0, 10}, {0, 11}}));
        connect(bond2.bondCon2, v1_v2.bondConJ1) annotation(
          Line(points = {{0, 29}, {0, 36}}));
        connect(v1_v2.bondConJ2, R2.bondCon) annotation(
          Line(points = {{24, 36}, {30, 36}}));
        connect(bond1.bondCon2, v2.bondConJ2) annotation(
          Line(points = {{34, 0}, {36, 0}}));
        connect(v2.bondConJ1, R.bondCon) annotation(
          Line(points = {{56, 0}, {60, 0}}));
        connect(v2.bondConJ3, L.bondCon) annotation(
          Line(points = {{46, -10}, {46, -14}}));
        annotation(
          Documentation(info = "<html>
      <p>This Bond Graph represents an RLC circuit without sources. The inertia (I) represents the inductance and is connected to a 0-junction along with a resistor, meaning they are in parallel. This parallel combination, the capacitor (C), and another resistor (R) are connected to a 1-junction, which enforces the same flow through them, representing a series connection.</p>
      </html>"),
          experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
      end RLCCircuit;

      model DCMotor "Permanent magnet DC motor"
        BondGraph.Components.Basic.Se Ua(E = 460) annotation(
          Placement(transformation(origin = {-76, 40}, extent = {{-10, -10}, {10, 10}})));
        BondGraph.Components.Basic.R Ra(R = 0.05) annotation(
          Placement(transformation(origin = {-46, 16}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
        BondGraph.Components.Basic.I La(I = 0.003) annotation(
          Placement(transformation(origin = {-46, 66}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
        BondGraph.Components.Basic.I J(I = 15) annotation(
          Placement(transformation(origin = {52, 42}, extent = {{-10, -10}, {10, 10}})));
        BondGraph.Components.Basic.R b(R = 1.1) annotation(
          Placement(transformation(origin = {26, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
        BondGraph.Components.Basic.J1 ia(N = 4) annotation(
          Placement(transformation(origin = {-46, 40}, extent = {{-10, -10}, {10, 10}})));
        BondGraph.Components.Basic.J1 w annotation(
          Placement(transformation(origin = {26, 42}, extent = {{-10, -10}, {10, 10}})));
        Components.Basic.GY gy(K = 6.76) annotation(
          Placement(transformation(origin = {-10, 40}, extent = {{-10, -10}, {10, 10}})));
      equation
        connect(Ua.bondCon, ia.bondConJ1) annotation(
          Line(points = {{-61, 40}, {-56, 40}}));
        connect(ia.bondConJ3, La.bondCon) annotation(
          Line(points = {{-46, 50}, {-46, 54}}));
        connect(ia.bondConJ4, Ra.bondCon) annotation(
          Line(points = {{-46, 30}, {-46, 28}}));
        connect(w.bondConJ2, J.bondCon) annotation(
          Line(points = {{36, 42}, {40, 42}}));
        connect(b.bondCon, w.bondConJ3) annotation(
          Line(points = {{26, 58}, {26, 52}}));
        connect(ia.bondConJ2, gy.bondCon1) annotation(
          Line(points = {{-36, 40}, {-32, 40}}));
        connect(gy.bondCon2, w.bondConJ1) annotation(
          Line(points = {{11, 40}, {14, 40}, {14, 42}, {16, 42}}));
        annotation(
          Documentation(info = "<html>
      <p>This Bond Graph represents a permanent magnet DC motor. The electrical side consists of an effort source (Se) representing the applied voltage, a resistor (R) for the armature resistance, and an inductor (I) representing the armature inductance, all connected to a 1-junction which enforces the same current. The GY element models the electromechanical energy conversion, linking the electrical domain to the mechanical one. On the mechanical side, the rotational inertia (I) represents the rotor and load, while the resistor (R) models the viscous friction losses.</p>
      </html>"),
          experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 0.002));
      end DCMotor;

      package SepExcMotor "Separately excited DC motor"
        model NLI "Non linear inertia"
          extends BondGraph.Interfaces.PassiveOnePort;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This Bond Graph component represents a nonlinear inductor. It includes a static relationship between the flow (current) and the generalized momentum (magnetic flux), defined by the values in the f_table and p_table.</p>
        </html>"),
            Icon(graphics = {Text(origin = {98, 6}, extent = {{-52, 92}, {52, -92}}, textString = "I")}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
        end NLI;

        model SepExcDCMotor "Separately excited DC motor (the magnetic flux as output)"
          BondGraph.Examples.SepExcMotor.NLIp nli(f_Table = {0, 3, 6, 9, 12, 15}, p_Table = {0, 200, 380, 480, 530, 545}) annotation(
            Placement(transformation(origin = {-20, -24}, extent = {{-10, -10}, {10, 10}})));
          BondGraph.Components.Basic.Se Ue(E = 184) annotation(
            Placement(transformation(origin = {-74, -24}, extent = {{-10, -10}, {10, 10}})));
          BondGraph.Components.Basic.R Re(R = 25.2) annotation(
          ...
          BondGraph.Components.Modulated.MGY mgy annotation(
          ...
          Components.Basic.J1 ia(N = 4) annotation(
            Placement(transformation(origin = {-46, 40}, extent = {{-10, -10}, {10, 10}})));
          Components.Basic.J1 w annotation(
            Placement(transformation(origin = {26, 42}, extent = {{-10, -10}, {10, 10}})));
          Components.Basic.J1 ie annotation(
            Placement(transformation(origin = {-44, -24}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
        equation
          connect(nli.y, gain.u) annotation(
            Line(points = {{-3, -24}, {17, -24}}));
          connect(Ua.bondCon, ia.bondConJ1) annotation(
            Line(points = {{-61, 40}, {-56, 40}}));
          connect(ia.bondConJ3, La.bondCon) annotation(
            Line(points = {{-46, 50}, {-46, 54}}));
          connect(ia.bondConJ2, mgy.bondCon1) annotation(
            Line(points = {{-36, 40}, {-34, 40}}));
          connect(ia.bondConJ4, Ra.bondCon) annotation(
            Line(points = {{-46, 30}, {-46, 28}}));
          connect(mgy.bondCon2, w.bondConJ1) annotation(
            Line(points = {{9, 40}, {13.5, 40}, {13.5, 42}, {16, 42}}));
          connect(w.bondConJ2, J.bondCon) annotation(
            Line(points = {{36, 42}, {40, 42}}));
          connect(b.bondCon, w.bondConJ3) annotation(
            Line(points = {{26, 58}, {26, 52}}));
          connect(Ue.bondCon, ie.bondConJ2) annotation(
            Line(points = {{-59, -24}, {-54, -24}}));
          connect(ie.bondConJ3, Re.bondCon) annotation(
            Line(points = {{-44, -34}, {-44, -38}}));
          connect(ie.bondConJ1, nli.bondCon) annotation(
            Line(points = {{-34, -24}, {-32, -24}}));
          connect(gain.y, mgy.u) annotation(
            Line(points = {{39, -24}, {44, -24}, {44, 12}, {-12, 12}, {-12, 31.5}}, color = {0, 0, 127}));
          annotation(
            Documentation(info = "<html>
        <p>This Bond Graph represents a separately excited DC motor. The model is divided into two electrical circuits: the field circuit and the armature circuit. The field circuit consists of a voltage source (Se), a resistance (R), and a non linear inductor (I) representing the excitation winding. The armature circuit includes another voltage source, a resistance, and an inductance, all connected to a 1-junction to ensure the same current flows through them. The GY element models the electromechanical conversion, and its coefficient is modulated by the magnetic flux, which is measured in the excitation intuctor.</p>
        </html>"),
            experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
        end SepExcDCMotor;

        model SepExcMotor "Separately excited DC motor(the magnetic flux calculated with the excitation current)"
          BondGraph.Examples.SepExcMotor.NLI nli(f_Table = {0, 3, 6, 9, 12, 15}, p_Table = {0, 200, 380, 480, 530, 545}) annotation(
            Placement(visible = true, transformation(origin = {-58, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
          BondGraph.Components.Basic.J1 ie(N = 4) annotation(
          ...
          BondGraph.Components.Basic.R b(R = 1.1) annotation(
            Placement(transformation(origin = {64, 68}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
          DSFLib.ControlSystems.Blocks.Components.Gain gain(K = 0.016) annotation(
            Placement(visible = true, transformation(origin = {70, -24}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
          BondGraph.Components.Sensors.FlowSensor ie_out annotation(
            Placement(visible = true, transformation(origin = {-34, -24}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
          BondGraph.Examples.SepExcMotor.LookUpTable lookUpTable(u_Table = {0, 3, 6, 9, 12, 15}, y_Table = {0, 200, 380, 480, 530, 545}) annotation(
        ...
        equation
          ...
          connect(Ra.bondCon, ia.bondConJ4) annotation(
            Line(points = {{0, 29.8}, {0, 33.8}}));
          connect(mgy.bondCon2, w.bondConJ1) annotation(
            Line(points = {{55.2, 42.2}, {57.2, 42.2}}));
          connect(w.bondConJ2, J.bondCon) annotation(
            Line(points = {{70.2, 42}, {74.2, 42}}));
          connect(ie.bondConJ4, Re.bondCon) annotation(
            Line(points = {{-57.8, -32}, {-57.8, -36}}));
          connect(ie.bondConJ1, Ue.bondCon) annotation(
            Line(points = {{-64.2, -24}, {-71, -24}}));
          connect(gain.y, mgy.u) annotation(
            Line(points = {{81.8, -24}, {89.8, -24}, {89.8, -2}, {33.8, -2}, {33.8, 34}}));
          connect(ie.bondConJ2, ie_out.bondCon) annotation(
            Line(points = {{-51.8, -24}, {-45.8, -24}}));
          connect(ie.bondConJ3, nli.bondCon) annotation(
          ...
          connect(b.bondCon, w.bondConJ3) annotation(
            Line(points = {{64, 56}, {64, 52}}));
          annotation(
            Documentation(info = "<html>
        <p>This Bond Graph represents a separately excited DC motor. The model is divided into two electrical circuits: the field circuit and the armature circuit. The field circuit consists of a voltage source (Se), a resistance (R), and a non linear inductor (I) representing the excitation winding. The armature circuit includes another voltage source, a resistance, and an inductance, all connected to a 1-junction to ensure the same current flows through them. The GY element models the electromechanical conversion, and its coefficient is modulated by the magnetic flux, which is calculated through a lookup table.</p>
        </html>"),
            Diagram,
            experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02));
        end SepExcMotor;

        model NLIp "Non linear inertia with generalized momentum as an output"
          extends BondGraph.Interfaces.PassiveOnePort;
          parameter Real[:] f_Table = {-2, -1, 0, 1, 2};
          parameter Real[:] p_Table = {-2, -1, 0, 1, 2};
          parameter Real p_0 = 0;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>This Bond Graph component represents a nonlinear inductor with a realoutput. It includes a static relationship between the flow (current) and the generalized momentum (magnetic flux), defined by the values in the f_table and p_table. The real output convert the generalized momentum into a real value.</p>
        </html>"),
        ...
        block LookUpTable "SISO LookUpTable"
          extends DSFLib.ControlSystems.Blocks.Interfaces.SISO;
        ...
        equation
          ...
          annotation(
            Documentation(info = "<html>
        <p>
        This model uses the LookUpTable function to return, as its output y, the result of performing a linear interpolation over a discrete set of data points (u_Table and y_Table) based on the input value u.
        </p>
        </html>"),
